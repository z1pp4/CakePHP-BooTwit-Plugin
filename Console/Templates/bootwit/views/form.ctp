...
<?php
		...
		foreach ($fields as $field) {
			if (strpos($action, 'add') !== false && $field == $primaryKey) {
				...
				echo "\t\techo \$this->Form->input('{$field}',array( 'error'=> array( 'attributes' => array('wrap' => 'div','class' => 'alert alert-error'))));\n";
			}
		}
		if (!empty($associations['hasAndBelongsToMany'])) {
			foreach ($associations['hasAndBelongsToMany'] as $assocName => $assocData) {
				echo "\t\techo \$this->Form->input('{$assocName}',array('error'=>'testAss') );\n";
			}
		}
		echo "\t?>\n";
?>
...
<?php
	echo "<?php echo \$this->Form->end(__('Submit', array('class' => 'btn btn-primary', 'div' => 'false')));?>\n";
?>
</div
</div>
